(function ($) {
    ...
    $s.DateTimeCommon = {
        ...
        getCurrentDate : function (offset, utcMode) {
            var currDate = $s.DateTimeCommon.getCurrentDateTime(offset, utcMode);
            currDate.setHours(0);
            currDate.setMinutes(0);
            return currDate;
        },
    
        //this returns current date and time according to time zone offset from utc and if it is already in utc mode.
        getCurrentDateTime: function (offset, utcMode) {
            ...
            if (utcMode) {
                ...
                if (offset) {
                    if (offset === 1) {
                        ...
                        currDate.setTime(currDate.getTime() + currDate.getTimezoneOffset() * 60 * 1000 + offset * 60 * 1000);
                    }
                }
            }

            // Set the current date and time in hours and minutes
